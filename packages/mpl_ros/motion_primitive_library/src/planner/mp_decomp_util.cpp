...
MPDecompUtil::MPDecompUtil(bool verbose)
{
...
void MPDecompUtil::setMap(const vec_Vec3f& obs, decimal_t r) {
  ENV_.reset(new MPL::env_decomp(obs, r));
}

bool MPDecompUtil::plan(const Waypoint &start, const Waypoint &goal) {
  path_.clear();
...
Polyhedra MPDecompUtil::getPolyhedra() {
  return ENV_->polyhedra();
}

vec_Ellipsoid MPDecompUtil::getEllipsoids() {
  return ENV_->ellipsoids();
}
