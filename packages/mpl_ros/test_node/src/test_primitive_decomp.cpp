...
#include <planner/mp_decomp_util.h>
...
std::unique_ptr<MPDecompUtil> planner_;
...
int main(int argc, char ** argv){
  ...
  ros::Publisher cloud_pub = nh.advertise<sensor_msgs::PointCloud>("distance_cloud", 1, true);
  ...
  planner_.reset(new MPDecompUtil(true));
  planner_->setMap(map_util->getCloud(), robot_radius); // Set collision checking function
  ...
  planner_->setMode(1, use_3d, start); // Set discretization with 1 and efforts
  //Planning thread!
  if(solve(start, goal)) {
    ...
    //Get time allocation
    std::vector<decimal_t> dts = planner_->getDts();
  ...
  decomp_ros_msgs::Ellipsoids es_msg = DecompROS::ellipsoids_to_ros(planner_->getEllipsoids());
